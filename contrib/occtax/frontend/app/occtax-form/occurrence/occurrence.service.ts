...
export class OcctaxFormOccurrenceService {
  ...
  private setObservables() {
    ...
    const $_occurrenceSub = this.occurrence.pipe(
      ...
      tap(
        (occurrence) =>
          (this.occtaxFormCountingsService.countings =
            occurrence.cor_counting_occtax),
      ),
    );

    const $_fieldsObservable =
      this.occtaxFormService.getGlobalAndAdditionalFields(["OCCTAX_OCCURENCE"]);
    ...
    combineLatest($_occurrenceSub, $_fieldsObservable, this.taxref)
      .pipe(
        filter(
          ([occurrence, additional_fields, taxref]) => occurrence !== null,
        ),
        map(([occurrence, additional_fields, taxref]) => {
          additional_fields =
            this.occtaxFormService.getAdditionalFieldsWithTaxref(
              additional_fields,
              taxref,
            );
          ...
          additional_fields.forEach((field) => {
            ...
            if (field.type_widget == "date") {
              ...
              if (
                typeof occurrence.additional_fields[field.attribut_name] !==
                "object"
              ) {
            ...
            if (
              occurrence.additional_fields[field.attribut_name] !== undefined
            ) {
      ...
      .pipe(
        map((DATA) => {
          return {
            ...
            non_digital_proof: this.occtaxParamS.get(
    ...
    if (
        ...
        .pipe(
          ...
          tap((occurrence) => {
            this.commonService.translateToaster(
              "info",
              "Occtax.Taxon.Messages.UpdateDone",
            );
      ...
      api = this.occtaxDataService.createOccurrence(id_releve, formValue).pipe(
        tap((occurrence) => {
          this.commonService.translateToaster(
            "info",
            "Occtax.Taxon.Messages.CreateDone",
          );
    ...
    this.occtaxDataService
      ...
      .subscribe(
        (confirm: boolean) => {
          ...
          this.commonService.translateToaster(
            "info",
            "Occtax.Taxon.Messages.DeleteDone",
          );
        },
        (error) => {
          this.commonService.translateToaster("error", "Errors.ErrorOccurs");
